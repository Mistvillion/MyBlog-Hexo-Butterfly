...
#### 6.5.1 ALU电路
*   能进行多种算术运算和逻辑运算的组合逻辑电路。
*   典型芯片：**74181** (4位ALU)。
*   控制端：$S_3S_2S_1S_0$ 选择运算功能，$M$ 选择算术/逻辑模式。

#### 6.5.2 快速进位链

为了解决多位加法器中**串行进位**延迟太大的问题，引入了**并行进位**（先行进位）。

**1. 串行进位 (Ripple Carry)**
*   高位的进位必须等待低位计算完成。
*   延迟 $T = n \times t_{adder}$。效率低。

**2. 并行进位 (Lookahead Carry)**
*   **原理**：进位 $C_{i+1}$ 只取决于输入 $A, B$ 和最低进位 $C_0$，而不依赖于 $C_i$。
*   **公式推导**：
    *   生成信号：$G_i = A_i B_i$ （只要A, B都为1，必产生进位）。
    *   传播信号：$P_i = A_i \oplus B_i$ （只要A, B有一个为1，低位进位就能传过去）。
    *   $C_{i+1} = G_i + P_i C_i$
*   **展开**：
    *   $C_1 = G_0 + P_0 C_0$
...
*   **特点**：所有进位 $C_1 \sim C_4$ 同时产生，延迟恒定（通常为2级门延迟）。

**3. 4位快速加法器**
*   利用上述逻辑，在一个模块内并行产生4位进位。
*   芯片：**74182** (先行进位发生器 CLA)。

**4. 多位快速加法器 (16位/32位)**
*   **单级先行进位**（组内并行，组间串行）：4个4位CLA串联。简单但不够快。
*   **多级先行进位**（组内并行，组间并行）：
    *   将16位分成4组。
    *   组内用CLA产生进位。
    *   组间也用类似CLA的逻辑（成组生成 $G^*$ 和成组传递 $P^*$）来并行产生组进位。
    *   **速度最快**。
